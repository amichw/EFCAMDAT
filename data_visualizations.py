...
def allDistFromLang(all_heats, langs):
    ...
    plt.bar(allDist.sum().sort_values().index, allDist.sum().sort_values().values)
    plt.yscale('log')
    save_close('dist_error_summed', 'Sum of distances by language (error-profile)', 'language', 'sum of distances (log scale)')
    return allDist
...
def saveHeatMap(mat, title='', x='', y='', fileName='temp.png'):
    ...
    plt.savefig(fileName.replace(' ', '_').replace(':', ''), dpi=80)
    plt.close()
...
def allDistLang2vec(langs):
    ...
    plt.bar(distL2vCS.sum().sort_values().index, distL2vCS.sum().sort_values().values)
    save_close('dist_lang2vec_summed', 'Sum of distances by language', 'language', 'sum of distances')
    return distL2vCS
...
def save_heat(heat, lang):
    ...
    fig.set_size_inches(22, 12, forward=False)
    # plt.tight_layout()
    plt.savefig(f'graphs/heat_{lang}.png'.replace(' ', '_').replace(':', ''), dpi=80)
    plt.close()
...
def save_err_profile_minus_avg_by_nationality(df, languages):
    ...
    save_heat(heat, ' avg by nationality')
...
def per_level_bar(df, levels):
    ...
    :return:
    """
    #  create multi graph plt, in each a bar plot of all levels for that error. manually.
    levels.sort()
    first = levels[0]
    all_mats = get_mat_vals(df[df[COL_LEVEL] == first])
...
if __name__ == '__main__':
    ...
    df=df[df[COL_N_ERROR]!='R:OTHER'] # found only one 'R:OTHER'
    df=df[df[COL_LEVEL]!='4']  # small DS
    df=df[df[COL_LEVEL]!='6']  # small DS
    print_to_log('loaded')
    df = df.drop(columns=['orig'])
    freq_errors, freq_langs, freq_levels = dataset_statistics(df)
    levels = freq_levels[COL_LEVEL].to_list()
    N_LANGUAGES = 20 # Number of languages to use for calculations
    all_languages = freq_langs[COL_LANG].to_list()[:N_LANGUAGES]
    # freq_err = freq_errors[COL_N_ERROR].to_list()[:20]
    df=df[df[COL_LANG].isin(all_languages)]
    # print(freq_langs)
    # print(freq_errors)
    N = 10  # Number of languages to use for graphs (N largest in dataset. discard the smaller ones.)
    langs = all_languages[:N]
    vis_whole_ds(df)
    save_err_profile_by_nationality(df, langs)
    save_err_profile_minus_avg_by_nationality(df, langs)
    save_err_profile_minus_avg_by_level(df, levels)
    save_err_profile_by_nationality_level(df, langs, levels)
    save_err_profile_by_level(df, levels)
    ...
    errors_dist = allDistFromLang(all_heats, all_languages)
    lang2vec_dist = allDistLang2vec(all_languages)  # objective no. 1 . complete!
    rank_correlation(errors_dist, lang2vec_dist, all_languages)

    df['pre'] = df[COL_N_ERROR].apply(lambda x: x.strip().split('->')[0])
    df['post'] = df[COL_N_ERROR].apply(lambda x: x.strip().split('->')[1])
    vis_words(df)
    vis_words(df, 'post', 'SYM')

    compare_all_dimitry(df)
